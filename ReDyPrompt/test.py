...
from forward_utils import (get_dynamic_adapted_text_embedding,
...
def get_predictions(
    ...
    image_dataloader = torch.utils.data.DataLoader(
        image_dataset, batch_size=2, shuffle=True
    )
    print(len(image_dataloader))
    support_proto = get_support_global_proto(model, image_dataloader, device)
    for input_data in tqdm(test_loader):
        ...
        epoch_text_feature =get_dynamic_adapted_text_embedding(support_proto, model, dataset, image_dataloader, det_feature, device)
        # calculate similarity and get prediction
        # cls_preds = []
        pred = det_feature @ epoch_text_feature
        # print(pred,'pred',pred.shape,'pred.shape')
        pred = (pred[:, 1] + 1) / 2
        # print(pred, 'pred', pred.shape, 'pred.shape')
...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="/media/code/AA-CLIP-TTA/AA-CLIP-main/HeRLB_AD_aaclip_1-shot")
    ...
    files = sorted(glob(args.save_path + "/image_adapter_20.pth"))
    ...
    for file in files:
        ...
        #         # )
        #         text_embeddings = get_dynamic_text_embeddings_with_support(model, args.dataset, class_name, device,
        #                                                  img_size, 1, 2, logger,
        #                                                  adapt_text, clip_model, support_loader=None)
        #     else:
        #         text_embeddings = get_adapted_text_embedding(
        #             clip_model, args.dataset, device
        #         )
        # ========================================================
        df = DataFrame(columns=["class name", "I-AUROC", "I-AP", "I-F1max", "P-AUROC", "P-AP", "P-F1max", "P-AUPRO"])
        ...
        for class_name, image_dataset in image_datasets.items():
            ...
            with torch.no_grad():
                ...
                masks, labels, preds, preds_image, file_names = get_predictions(
                    ...
                    device=device,
                    img_size=args.img_size,
                    dataset=args.dataset,logger=logger,
                )
            # ========================================================
            if args.visualize:
                visualize(
