...
import warnings
# from forward_utils import supcon_loss,margin_separation_loss
...
def train_text_adapter(
    ...
    for epoch in range(start_epoch, text_epoch):
        ...
        for input_data in tqdm(train_loader):
            ...
            t0 = F.normalize(epoch_text_feature[:, :, 0],dim=1)
            t1 = F.normalize(epoch_text_feature[:, :, 1],dim=1)

            t2 = torch.cat([t0, t1], dim=0)
            print(t0.shape, t1.shape,t2.shape)
            lables = torch.tensor([0]*len(t0) + [1]*len(t1), dtype=torch.long)
            # forward image
            with torch.no_grad():
            ...
            for f in patch_features:
                ...
                loss_seg = calculate_seg_loss(patch_preds, mask)
                # print(loss_seg)
                orthogonal_loss = (
                ...
                cos01 = (t0 * t1).sum(dim=1)
                sep_margin = 0.1
                loss_sep = torch.relu(cos01 - sep_margin).pow(2).mean()

                # loss_sep = supcon_loss(t2, lables,tau=0.07)
                print(orthogonal_loss)
                loss_t_p = loss_seg + orthogonal_loss
                # print(loss_t_p)
                patch_preds_rank = patch_preds  # 用最后一次循环算出来的
                # 假设 patch_preds_rank 形状 [B,2,H,W]，其中 [:,0] normal score, [:,1] anomaly score
                sn = patch_preds_rank[:, 0]  # [B,H,W]
                sa = patch_preds_rank[:, 1]

                m = mask.float()
                # 如果 mask 是 [B,1,H,W]，先 squeeze
                ...
                rank_margin = 0.2  # 可调：0.1~0.5
                # 缺陷区域：sa 应该比 sn 大
                loss_rank_anom = torch.relu(
                    rank_margin - (sa[anom_region] - sn[anom_region])).mean() if anom_region.any() else 0.0
                # 正常区域：sn 应该比 sa 大
                loss_rank_norm = torch.relu(
                    rank_margin - (sn[norm_region] - sa[norm_region])).mean() if norm_region.any() else 0.0

                rank_weight = 0.5  # 可调
                loss = loss_t_p + rank_weight * (loss_rank_anom + loss_rank_norm)
                # print(loss)
...
def train_image_adapter(
    ...
    for epoch in range(start_epoch, image_epoch):
        ...
        for input_data in tqdm(train_loader):
            ...
            logit_scl = model.clipmodel.logit_scale.exp()
            cls_preds = torch.matmul(det_feature, text_embeddings)[:, 0]
            # print(cls_preds,label,logit_scl * F.cross_entropy(cls_preds, label))
            # print(epoch_text_feature.shape)
            # print(F.cosine_similarity(epoch_text_feature[1,:,0], epoch_text_feature[0,:,1], dim=0).item())
            loss += logit_scl * F.cross_entropy(cls_preds, label)
            for f in patch_features:
                # text-image alignment
                patch_preds = calculate_similarity_map(f, text_embeddings, img_size)
...
def main():
    ...
    # training
    parser.add_argument("--dataset", type=str, default="HeRLB_AD")
    parser.add_argument(
        "--training_mode",
        type=str,
        default="full_shot",
    ...
    parser.add_argument("--shot", type=int, default=1, help="number of shots (0 means full shot)")
    ...
    parser.add_argument("--image_epoch", type=int, default=0, help="epochs for stage2")
    ...
    parser.add_argument("--save_path", type=str, default="/media/code/AA-CLIP-TTA/AA-CLIP-main/xiaorong_1_HeRLB_AD_aaclip_full_shot")
    # hyper-parameters
    parser.add_argument("--text_norm_weight", type=float, default=0.3)
